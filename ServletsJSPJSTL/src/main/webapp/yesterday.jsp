<%-- 
    Document   : yesterday
    Created on : 13 mar 2021, 16:45:00
    Author     : woodpecker
--%>
...
<html>
    ...
    <body>
        ...
        <c:if test="${!empty membersBean.members}">
            <table <c:if test="${membersBean.getIsVisibleEdges().equals(Boolean.TRUE)}">border</c:if>>
                <tr><th>The Beatles</th></tr>
            <c:forEach var="member" items="${membersBean.members}">
                    <tr><td>${member}</td></tr>
            </c:forEach>
            </table>
        </c:if>
