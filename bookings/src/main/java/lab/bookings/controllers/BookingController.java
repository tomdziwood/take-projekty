...
public class BookingController {
    ...
    @PostMapping
    public String create(Booking booking, Errors errors, Model model) {
        String view;
        if (errors.hasErrors()) {
            model.addAttribute("bookings", bookingRepository.findAll());
            view = "bookings";
    ...
    public String delete(@RequestParam(required = true, name = "id") Long id, Model model) {
        String view;

        Booking deletingBooking = bookingRepository.getBookingById(id);
        if (deletingBooking == null) {
            model.addAttribute("booking", new Booking());
            model.addAttribute("noBookingToDelete", true);
            model.addAttribute("bookings", bookingRepository.findAll());
            view = "bookings";
        } else {
